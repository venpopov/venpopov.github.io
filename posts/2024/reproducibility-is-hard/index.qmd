...
description: "I've spent more time debugging my workflow than actually doing research. There has to be a better way."
...
But even if all the quirks were ironed out, and the process streamlined, another issue, that I rarely see discussed, is just how much more "irrelevant" artefacts are produced in the codebase. When I look [at my best attempts to implement the full workflow](https://venpopov.github.io/ltm-time-benefit/), I am a bit paralized by the sheer amount of files and code in my repositories that is not directly related to the substance of the research. There are layers upon layers of abstraction, scaffolding, configuration files, and helper functions that are just necessary for the workflow to function.
